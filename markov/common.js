...
myApp.directive('stDiagram', function($compile) {
  function link(scope, el, attr) {
    ...
    var currentState = 0;
    var nodeName = ["Start", "Branch", "Call Centre", "ATM", "Online Banking", "Mobile App", "Tablet App", "Ad Words", "Television", "Website", "Newspaper", "Push Notification", "Email", "Letter", "Outbound Call", "Social Media", "Affiliate Marketing", "Sale", "No Sale"];
	var saleCount = document.getElementById("saleCount");
	var saleCountNum = 0
	var nonSaleCount = document.getElementById("nonSaleCount");
	var nonSaleCountNum = 0
	var nonSaleCountPer = 0
	var saleCountPer = 0 
	...
	function loop() {
      ...
      currentStateG
		...
		console.log(nodeName[currentState]);
		console.log(nodeName[nextState]);
		if(nodeName[currentState] == 'Sale') {
			console.log('Restart');
			currentState = 0;
			saleCountNum += 1;
			saleCountPer = Math.round((100 * saleCountNum) / (saleCountNum + nonSaleCountNum));
			nonSaleCountPer = Math.round((100 * nonSaleCountNum) / (saleCountNum + nonSaleCountNum));
			saleCount.innerHTML = "Sale: " + saleCountNum + " (" + saleCountPer + "%)";
			nonSaleCount.innerHTML = "No Sale: " + nonSaleCountNum + " (" + nonSaleCountPer + "%)";
		}
		else if(nodeName[currentState] == 'No Sale') {
			console.log('Restart');
			currentState = 0;
			nonSaleCountNum += 1;
			saleCountPer = Math.round((100 * saleCountNum) / (saleCountNum + nonSaleCountNum));
